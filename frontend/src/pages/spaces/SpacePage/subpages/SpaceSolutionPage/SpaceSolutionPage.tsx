...
export const SpaceSolutionPage: FC<Props> = typedMemo(
    function SpaceSolutionPage({
        className,
        ...
        const solution = useMemo(
            () => (isOrganizer ? organizerSolution : expertSolution),
            [isOrganizer, organizerSolution, expertSolution],
        );

        const { data: issue } = useGetIssue(issueId!);
        const [criteriaFilters] = useListFilters();
        const { data: issueCriteria } = useGetIssueCriteria(
      solution!.issueId,
      criteriaFilters,
      {
        ...
        const content = useMemo(() => {
            switch (activeSection) {
                ...
                case ActiveSection.Criteria:
                    return (
                        <SolutionCriteria criteria={issueCriteria?.entityList ?? []} />
                    );
        ...
        return (
            ...
            Сдать до: {getDateFromISO(issue?.submitDeadlineDateUtc ?? '')}
                    </Text>
                    <Text className={getBemClasses(styles, 'deadline')}>
            Оценить до: {getDateFromISO(issue?.assessmentDeadlineDateUtc ?? '')}
                    </Text>
                </FlexContainer>
                ...
                >
                    ...
                    {canReview && hasReview === undefined
                        ? (
                            <NavLink
                                to={SpaceRouter.EstimateTaskWork(
                                    spaceId ?? '',
                                    issueId ?? '',
                                    solutionId ?? '',
                                )}
                            >
                                <Button>Оценить работу</Button>
                            </NavLink>
                        )
                        : null}
                </FlexContainer>
